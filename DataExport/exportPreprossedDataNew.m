function handles = exportPreprossedDataNew(Decon_param, name_cell, folder_cell,Background,iIRF,handles,show_waveform_flag)
...
save_folder = ['PreprocessedData' datestr(t,formatOut)];
...
savepath = fullfile(folder_cell{1},save_folder); % set saving pass
sampleName = strsplit(folder_cell{1},'\');
sampleName = sampleName{end};
%% loop for each line
for i=1:num_of_batches
    ...
    name = strcat(name_cell{i},'PreprocessedData'); % creat saving file name
    ...
    temp_text = sprintf('Exporting Line %d ',i);
    ...
    save(name,'rawData','BG_removed','truncateDataAllCell','timeStamp','spaceStamp','gain_factor', ...
        'gain_list','truncatedDataArray','BG_scale_factor');
    fileID = fopen([name '_HV.txt'],'w');
    fprintf(fileID,'%6.2f\r\n',BG_scale_factor);
    fclose(fileID);
...
save(strcat(sampleName,'_ImageCube'),'truncatedDataAll','CH1DataAll','CH2DataAll','CH3DataAll','CH4DataAll');
