...
addpath(genpath(pwd))
%% load in data (note: save folder needs to be created manually before)
% root = '';
root = 'Y:\V2_Sacramento_Database\Da Vinci Robot Study (100 patients)\Data_100_Patient_Study\Subject_119';
...
[Reffile,Refpath] = uigetfile([root '\*.jpeg'],'Please select the reference image');
% savePath = 'C:\Users\Xiangnan\Box Sync\FLImBrush vs V4\20210505\FLImBrush\20210505_neuro024\AugmentedImages';
savePath = 'C:\Users\Katjana Ehrlich\Documents\Data\Image_Reconstruction'; %Create folder before 
%% load in data
...
% read in new positions from mat file 
fName = strtok(Posfile, '.');
pos_new = eval([fName '(:, 1:2)']);

% read in new positions from yolov5 method
%pos_new=table2array(after20230322PEMrun47(:,3:4));
...
%% Plot overlay
h1 = figure();
histogram(output.lt1); hold on;
histogram(output.lt2); hold on;
histogram(output.lt3);
xlabel('Fluorescence Lifetime (ns)')
ylabel('Frequency')
legend('Ch1 LT', 'Ch2 LT', 'Ch3 LT');
savefig(h1, [root '\Image_Reconstruction\' name '_ReconFig_hist2.fig']),

f1 = figure;
%imshow(output.img); hold on; scatter(output.xx, output.yy, 25, output.lt2, 'filled');
imshow(refimg); 
hold on; 
scatter(output.xx, output.yy, 30, output.lt1, 'filled')
caxis(gca,[0.5 7.5]);
savefig(f1, [root '\Image_Reconstruction\' name '_ReconFig_LT12.fig']),
...
imshow(refimg); 
hold on; 
scatter(output.xx, output.yy, 30, output.lt2, 'filled')
caxis(gca,[0.5 7.5]);
savefig(f2, [root '\Image_Reconstruction\' name '_ReconFig_LT22.fig']),

f3 = figure;
%imshow(output.img); hold on; scatter(output.xx, output.yy, 25, output.lt2, 'filled');
imshow(refimg); 
hold on; 
scatter(output.xx, output.yy, 30, output.lt3, 'filled')
caxis(gca,[0.5 7.5]);
savefig(f2, [root '\Image_Reconstruction\' name '_ReconFig_LT32.fig']),
