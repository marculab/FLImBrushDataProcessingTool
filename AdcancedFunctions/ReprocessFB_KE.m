...
% Xiangnan 09/09/2022
function ReprocessFB(input_mat_name, saveRootPath)
...
bgLow = Ch1DataObj_old.bgLow;
%bgLow = Ch1DataObj_old.bgLow/4*5;
%bgLow = round(bgLow);
bgHigh = Ch1DataObj_old.bgHigh;
...
LaguerreOrder = 12;
alpha = 0.916;
%expOrder = Ch1DataObj_old.expDeconObj.order; not needed  
...
%% ----------------------------saving data---------------------------------
[~,runName,~] = fileparts(name); % get run name
...
%save(saveFileFullPath, 'dataInfoObj','Ch1DataObj','Ch2DataObj','Ch3DataObj','-v7.3');

%% save Decon Lite
%saveDeconLite(Ch1DataObj,Ch2DataObj,Ch3DataObj,saveFileFullPath)

saveas(fCh1, fullfile(saveRootPath,saveFolderName, 'fCh1_figure'), 'fig');
saveas(fCh2, fullfile(saveRootPath,saveFolderName, 'fCh2_figure'), 'fig');
saveas(fCh3, fullfile(saveRootPath,saveFolderName, 'fCh3_figure'), 'fig');
...
plotStep = 100;
fBGremove = plotBGRemoveFigures(Ch1DataObj, Ch2DataObj, Ch3DataObj, plotStep, bgLow, bgHigh, DigitizerNoise, ChWidthInPoints);

saveas(fBGremove,fullfile(saveRootPath,saveFolderName,'BGRemoval'),'fig');

snrVSlt = plotsnrVSlt(Ch1DataObj, Ch2DataObj, Ch3DataObj, [1000 1000], [25 65]);
saveas(snrVSlt,fullfile(saveRootPath,saveFolderName,'SNRvsLT'),'fig');
