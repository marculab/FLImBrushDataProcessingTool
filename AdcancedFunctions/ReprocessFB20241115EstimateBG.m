...
% Xiangnan 2024-10-30
function ReprocessFB20241115EstimateBG(input_mat_name,SaveHardDriveLetter,apd1Obj,apd2Obj,apd3Obj,apd4Obj,expOrderIn)
...
filepath = filepath{1};
DateString = char(datetime(2025,05,01));
filepath = [filepath '_' DateString 'Tolx10000'];
...
if expOrder>0
runDeconExp(Ch1DataObj,expOrder,[],[],[],Ch1DataObj.exclude);
end
...
if expOrder>0
runDeconExp(Ch2DataObj,expOrder,[],[],[],Ch2DataObj.exclude);
end
...
if expOrder>0
runDeconExp(Ch3DataObj,expOrder,[],[],[],Ch3DataObj.exclude);
end
...
if numOfChannel==4
    ...
    if expOrder>0
        runDeconExp(Ch4DataObj,expOrder,[],[],[],Ch4DataObj.exclude);
    end
    nexttile
    % idxTemp = find(max(Ch3DataObj.dataT)>0.4&max(Ch3DataObj.dataT)<0.6);
    % idxTemp = idxTemp(50);
...
saveName = fullfile(filepath,[name '.mat']);
if numOfChannel==4
    Ch4DataObj.toSingle();
    save(saveName,'dataInfoObj','Ch1DataObj','Ch2DataObj','Ch3DataObj','Ch4DataObj','EOP_H1G','EOP_H1S','SP_G','SP_S','-v7.3')
    saveDeconLite(Ch1DataObj,Ch2DataObj,Ch3DataObj,Ch4DataObj,EOP_H1G,EOP_H1S,SP_G,SP_S,saveName)
else
    save(saveName,'dataInfoObj','Ch1DataObj','Ch2DataObj','Ch3DataObj','EOP_H1G','EOP_H1S','SP_G','SP_S','-v7.3')
    saveDeconLite(Ch1DataObj,Ch2DataObj,Ch3DataObj,[],EOP_H1G,EOP_H1S,SP_G,SP_S,saveName)
end
